    ...
    public partial class Dangky : System.Web.UI.Page
    {
        protected void Page_Load(object sender,EventArgs e)
        {
            if (IsPostBack)
            {
                ...
                bool check = true;
                if (username!=""&&email!="" && sdt != "" && password!=""&&repassword!="")
                {
                    ...
                    if (check)
                    {
                        ...
                        NguoiDung newUser = new NguoiDung(username,email,password,repassword, sdt);
                        users.Add(newUser);
                        Application["Users"]=users;
                        btn_sl.InnerHtml = "số tài khoản là: " +dem;
